...
HITS="${HITS:-100}"
SORT="${SORT:-name}"
OUTPUT_DIR="${OUTPUT_DIR:-./data/actress}"
...
fetch_page() {
  local initial="$1" hits="$2" offset="$3"
...
for initial in "${INITIALS[@]}"; do
  ...
  for ((p=1; p<=pages; p++)); do
    offset=$(( (p - 1) * HITS + 1 ))
    json=$(fetch_page "$initial" "$HITS" "$offset")
    ...
    jq -c '.result.actress[] | {id: (.id|tonumber?), name, ruby}' <<<"$json" >>"$TMP_ALL" || true
...
# 重複を排除して保存
jq -s 'unique_by(.id)' "$TMP_ALL" >"${OUTPUT_DIR}/actresses_summary.json"
total_actresses=$(jq 'length' "${OUTPUT_DIR}/actresses_summary.json")
rm "$TMP_ALL"

echo "[DONE] ${total_actresses}件の女優データを保存しました: ${OUTPUT_DIR}/actresses_summary.json"
